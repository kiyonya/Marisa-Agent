...
export default class ToolGroup<P extends ToolkitPermissions = ToolkitPermissions> {
    ...
    public registeredTools: (LocalTool<any, any> | MCPTool<any, any>)[] = []
    ...
    public tool<ToolParams extends Record<string, any> = Record<string, any>, ToolResult = any>(toolName: string, description: string, executor: (params: ToolParams,permission?: PermissionAsker) => Promise<ToolResult> | ToolResult, paramsSchema: Record<keyof ToolParams, z.ZodTypeAny>, returnSchema?: z.ZodAny): LocalTool<ToolParams, ToolResult> {

        const localTool = new LocalTool<ToolParams, ToolResult>(this.fnameWithNamespace(toolName), description, executor, paramsSchema, returnSchema)
