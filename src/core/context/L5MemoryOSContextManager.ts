...
import { ModelContextManager, CategoryMemory, CategoryMemoryMetadata, MemoryCategoryAllowedType } from "./ModelContextManager";
import LocalTool from "../tool/LocalTool";
import RecursiveCharacterTextSplitter from "../splitter/RecursiveCharacterTextSplitter";
import MemorySearchDatabase from "../db/MessageSearchDatabase";
...
export interface L5MemoryOSConfig {
    /**
    ...
    /**
     * @description when query,filter the relevant memories by distance,default 0.2,which means only the memories with distance less than 0.2 will be returned
     * @default 0.2
     */
    vectorDistanceThreshold?: number
    /**
     * @description when query,filter the relevant memories from message search database by BM25 score,default 0.5,which means only the memories with BM25 score greater than 0.5 will be returned
     * @default 0.5
     */
    searchMessageBM25ScoreThreshold?: number
    /**
 ...
 * 
 * @description `L5MemoryOSContextManager` は `ModelContextManager` を継承した長期的な記憶管理クラスです。会話セッションを非同期に統合し、BM25テキストデータベースとベクターデータベースの両方に書き込むことで、キーワード検索とセマンティック検索の両方をサポートします。クエリ時には、ユーザーの入力に基づいてホットメモリ、ベクターメモリ、カテゴリメモリ（ユーザー／フィードバック／リファレンス）を統合し、同時に大規模言語モデルに対して `load_category_memory` および `search_memory` ツールを注入します。これによりモデルは必要に応じて履歴メモリをロードまたは検索できるようになり、セッションをまたいだ永続的で構造化された検索可能なコンテキスト拡張を実現します。
 */

export default class L5MemoryOSContextManager extends ModelContextManager {
    ...
    private consolidateChatModel: Model | null = null
    private vectorStore: VectorStore<Metadata>
    private messageSearchDB: MemorySearchDatabase
    ...
    public config?: L5MemoryOSConfig

    constructor(consolidateChatModel?: Model, embeddingModel?: EmbeddingModel, embeddingDimension: number = 512, vectorStore?: VectorStore<Metadata>) {
        super()
        ...
        this.messageSearchDB = this.createMessageSearchDB()
        this.vectorStore = vectorStore || this.createEmptyVectorStore<Metadata>(embeddingDimension)
        if (this.vectorStore) {
            const storeDimension = this.vectorStore.dimemsion
            if (storeDimension !== embeddingDimension) {
                throw new Error(`Vector store dimension ${storeDimension} does not match embedding dimension ${embeddingDimension}`)
            }
        }
    ...
    public override async put(session: Marisa.Chat.Completion.CompletionSession, withHistory?: Marisa.Chat.Completion.CompletionSession[], sessionPutCallback?: () => void): Promise<any> {
        ...
        const meta = this.extractMessages(session)
        await this.insertBM25Search(meta)
    ...
    public override async query(userPrompt: string): Promise<[sessions: Marisa.Chat.Completion.CompletionSession[], promptAddition: string]> {
        ...
        const maxQueryMemoryLength = this.config?.maxQueryMemoryLength ?? 10
        let metadatas: Metadata[] = await this.queryRelevantCombined(userPrompt, maxQueryMemoryLength) || []
        ...
        const categoryMemoryIndexPrompt: string = categoryMemoryIndex ? `## 当前的记忆索引有\n${categoryMemoryIndex}\n使用${this.injectLoadCategoryMemoryToolName}来加载` : '## 当前还没有记忆索引'
    ...
    public configure(config: Partial<L5MemoryOSConfig>) {
        this.config = {
            ...this.config,
            ...config
        }
    ...
    private async consolidate(sessions: Marisa.Chat.Completion.CompletionSession[]): Promise<void> {
        ...
        await Promise.all([this.insertEmbeddingSearch(metadatas), this.consolidateChatModel ? this.modelUpdateCategoryMemory(this.consolidateChatModel, metadatas) : Promise.resolve()])
    }

    //insert
    private async insertBM25Search(metas: Metadata[]): Promise<void> {
        if (this.messageSearchDB) {
            try {
                this.messageSearchDB.beginTransaction()
                for (const meta of metas) {
                    this.messageSearchDB.insert(meta.role, meta.content, meta.time)
                }
                this.messageSearchDB.commitTransaction()
            } catch (error) {
                this.messageSearchDB.rollbackTransaction()
            }
        }
    }

    private async insertEmbeddingSearch(metas: Metadata[]): Promise<void> {
        ...
        for (const meta of metas) {
            ...
            const chunks = rcharSplitter.splitText(content)
            for (const chunk of chunks) {
                events.push({ ...meta, content: chunk })
            }
        }

        try {
            const inputs = events.map(i => i.content)
            const embeddings = await this.embeddingModel.embedding(inputs, this.embeddingDimension)
            ...
            for (const embd of datas) {
                ...
                const raw = events[dindex]
                const vectors = embd.embedding || new Array<number>(this.embeddingDimension).fill(0)
                const f32 = new Float32Array(vectors)
                if (raw?.content && raw.time && f32) {
                    inserts.push({
                        metadata: {
                            content: raw.content,
                            time: raw.time,
    ...
    private async modelUpdateCategoryMemory(model: Model, metas: Metadata[]) {
        ...
        const updateTool = new LocalTool<{ type: MemoryCategoryAllowedType, name: string, description: string, content: string }>('create_or_update_memory', updateToolDescription, ({ type, name, description, content }) => {
            ...
            type: z.enum(L5MemoryOSContextManager.memoryCategories),
            name: z.string(),
        ...
        **内容应该存储具体的事实信息，用完整的句子描述**
        ...
        ## 现在已经存储的子记忆索引如下
        ${this.buildMemoryCategoriesIndex()}
    ...
    private async queryRelevantMetadatasFromVectorStore(query: string): Promise<Metadata[]> {

        const metadatas: Metadata[] = []
        if (this.embeddingModel && this.vectorStore) {
            const embeddings = await this.embeddingModel.embedding(query, this.embeddingDimension)
            const vectors = embeddings.data[0]?.embedding || new Array<number>(this.embeddingDimension).fill(0)
            const f32array = new Float32Array(vectors)
            const relevants = await this.vectorStore.search(f32array, undefined, { limit: 20, orderBy: 'distance' })
            for (const relevant of relevants) {
                const distance = relevant.distance
                const metadata = relevant.metadata
                if (metadata && distance < (this.config?.vectorDistanceThreshold ?? 0.8)) {
    ...
    public async queryRelevantMetadatasFromSearchStore(query: string) {

        const metadatas: Metadata[] = []
        if (this.messageSearchDB) {
            const queries = this.messageSearchDB.search(query, 20)
            for (const query of queries) {
                if (query.score > (this.config?.searchMessageBM25ScoreThreshold ?? 0.6)) {
                    const metadata: Metadata = {
                        content: query.content,
                        role: query.role as AllowStoreRole,
    ...
    public async queryRelevantCombined(query: string, limit: number = 10): Promise<Metadata[]> {
        const vec = await this.queryRelevantMetadatasFromVectorStore(query)
        const bm25 = await this.queryRelevantMetadatasFromSearchStore(query)
        const vecCount = Math.floor(limit * 0.7)
        const bm25Count = limit - vecCount
        const combined = [...vec.slice(0, vecCount), ...bm25.slice(0, bm25Count)].sort((a, b) => b.time - a.time)
        return combined
    }

    private async queryRelvantMemoryFromCategoryMemory(queryKeywords: string[]): Promise<{ type: string; name: string; }[]> {
    ...
    private createModelInjectTools() {
        ...
        const searchMemoryTool = new LocalTool<{ query: string[] }>(this.injectSearchMemoryToolName, '根据关键词搜索相关记忆', async ({ query }): Promise<string> => {
            ...
            const maxQueryMemoryLength = this.config?.maxQueryMemoryLength ?? 10
            let matchMixinDatabaseMemories = await this.queryRelevantCombined(query.join(' '), maxQueryMemoryLength)
            let matchCategoryMemories = await this.queryRelvantMemoryFromCategoryMemory(query)

            if (!matchMixinDatabaseMemories.length && !matchCategoryMemories.length) {
                return '没有找到相关的记忆，你现在需要去询问用户相关的内容活着继续对话，用户的回答会被记录，这样你下次就可以搜索到相关记忆了，请不要调用工具去盲目查找！'
            }

            let dbMemories: string = matchMixinDatabaseMemories.length ? `
            ## 数据库相关记忆\n 
            ${matchMixinDatabaseMemories.map(i => `- 记忆角色:${i.role}  记忆时间:${this.semantifyTimestamp(i.time)}  记忆内容:${i.content}\n`).join('')}` : ''

            let recommendCategoryMemories = matchCategoryMemories.length ? `
            ## 另外找到可能相关的类型记忆，你可以使用${this.injectLoadCategoryMemoryToolName}工具调用查看
            ${matchCategoryMemories.map(i => `- 记忆类型:${i.type}  记忆名称:${i.name}\n`)}` : ''
